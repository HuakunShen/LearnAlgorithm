...
def test(test_times: int, lst_length: int):
    for i in range(test_times):
        ...
        assert real_num_inversion == count_inversion_divide_conquer_wrapper(lst), "Test Failed from test()"
    print("Testing {0} times with random lists of length {1} has passed.".format(test_times, lst_length))
